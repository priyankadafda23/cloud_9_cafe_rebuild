...
$user_name = $auth->getUserName() ?? 'Guest';
...
// Pending orders
$pendingOrders = $db->select('cafe_orders', ['user_id' => $user_id, 'status' => 'Pending']);
$stats['pending_orders'] = count($pendingOrders);

// Reward points
$user = $db->selectOne('cafe_users', ['id' => $user_id]);
$stats['reward_points'] = $user['reward_points'] ?? 0;

// Cart count
$cart = $db->select('cafe_cart', ['user_id' => $user_id]);
$stats['cart_items'] = count($cart);

// Recent orders - get last 5
$recentOrders = $db->select('cafe_orders', ['user_id' => $user_id], ['order_date' => 'DESC'], 5);
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            <?php foreach ($recentOrders as $order): 
                                // Get first item for this order
                                $orderItems = $db->select('cafe_order_items', ['order_id' => $order['id']], null, 1);
                                $firstItem = $orderItems[0] ?? null;
                                $menuItem = null;
                                if ($firstItem) {
                                    $menuItem = $db->selectOne('menu_items', ['id' => $firstItem['menu_item_id']]);
                                }
                            ?>
                            <tr>
                                <td class="ps-4">
                                    <div class="d-flex align-items-center">
                                        <div class="flex-shrink-0">
                                            <div class="bg-light rounded p-2">
                                                <img src="<?php echo $menuItem && $menuItem['image'] ? '../assets/images/' . $menuItem['image'] : 'https://via.placeholder.com/50'; ?>" 
                                                     alt="" style="width: 40px; height: 40px; object-fit: cover; border-radius: 8px;">
                                            </div>
                                        </div>
                                        <div class="ms-3">
                                            <h6 class="mb-0 fw-medium"><?php echo htmlspecialchars($menuItem['name'] ?? 'Order Item'); ?></h6>
